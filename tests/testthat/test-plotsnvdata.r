...
test_that(desc = 'plot_snv_data test', code = {
    ...
    plots <- plot_snv_data(seurat_object = processed_data$SeuratObject,
                        ...
                        cell_border = 0,
                        save_each_plot = TRUE)
    expect_false(is.null(plots$histograms))
    expect_false(is.null(plots$N_sceSNVs))
    expect_false(is.null(plots$N_VARreads))
    expect_false(is.null(plots$N_REFreads))
    expect_false(is.null(plots$Total_VAF_RNA))
    expect_false(is.null(plots$Mean_VAF_RNA))
    expect_false(is.null(plots$`Cell types (scType)`))
})